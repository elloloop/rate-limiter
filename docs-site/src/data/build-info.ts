...
export function getEditUrl(pageUrlPath: string): string {
  const astroPath = pageUrlToAstroPath(pageUrlPath);
  if (!astroPath) {
    return "https://github.com/elloloop/rate-limiter/tree/main/docs-site";
  }
  const rel = path.relative(REPO_ROOT, astroPath);
  return `https://github.com/elloloop/rate-limiter/edit/main/${rel}`;
}

function pageUrlToAstroPath(pageUrlPath: string): string | null {
  // pageUrlPath looks like "/rate-limiter/docs/quickstart/" or
  // "/rate-limiter/" for the root. Strip the base and the trailing slash.
  const base = "/rate-limiter";
...
export async function getGithubStars(): Promise<string | null> {
  ...
  try {
    ...
    const res = await fetch(
      "https://api.github.com/repos/elloloop/rate-limiter",
      {
